...
Autonomous indoor quadcopter flight using a Vicon motion-capture system for positioning — no GPS, no satellites, fully indoors. Built for the "Design and Control of UAVs" university project.

What it does
Stable autonomous hover — takes off and holds position within ±5 cm
Upward spiral trajectory — 1 m radius, climbing 0.5 m → 1.5 m over 3 windings, flown autonomously
All positioning comes from a Vicon motion-capture system, injected into the flight controller as if it were GPS — so the drone knows exactly where it is indoors.
Hardware
...
The key idea: ArduPilot's EKF is told to treat the Vicon feed as its GPS source (GPS_TYPE = 14), so all the normal autonomous-flight machinery works indoors with centimetre-accurate motion-capture data instead of satellites.
...
The Pixhawk connects over USB (/dev/ttyACM0); the Vicon feed streams over the lab network.
